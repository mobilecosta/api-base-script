...
describe('Integration Mock Routes (/api)', () => {
  it('should return dictionary columns for alias', async () => {
    const res = await request(app).get('/api/dictionary/browse/columns/Z10');
  ...
  it('should return paginated browse items', async () => {
    const res = await request(app).get('/api/dictionary/browse/items/Z10?page=1&pageSize=1');
    expect(res.status).toBe(200);
  ...
  it('should return dictionary data for positioned item payload', async () => {
    ...
    const res = await request(app).get(`/api/dictionary/data/Z10/${raw}`);
    expect(res.status).toBe(200);
    expect(res.body.Z10_COD).toBe('PLAT001');
  });

  it('should return initializer for alias', async () => {
    const res = await request(app).get('/api/dictionary/initializer/Z11');
    expect(res.status).toBe(200);
  ...
  it('should echo payload in trigger endpoint', async () => {
    ...
    const res = await request(app).post('/api/dictionary/trigger/Z10_DESC').send(payload);
    expect(res.status).toBe(200);
    expect(res.body.Z10_COD).toBe('AAA');
  });

  it('should return lookup entries', async () => {
    const res = await request(app).get('/api/lookup/SA1?filter=000001');
    expect(res.status).toBe(200);
  ...
  it('should return lookup by id', async () => {
    const res = await request(app).get('/api/lookup/SA1/000001');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
  ...
  it('should execute platforms CRUD flow', async () => {
    ...
    const created = await request(app).post('/api/platforms').send({
      Z10_COD: code,
    ...
    const updated = await request(app).put(`/api/platforms/${code}`).send({
      Z10_DESC: 'Plataforma Atualizada'
    });
    expect(updated.status).toBe(200);
    expect(updated.body.Z10_DESC).toBe('Plataforma Atualizada');

    const deleted = await request(app).delete(`/api/platforms/${code}`);
  ...
  it('should execute shipping program CRUD flow', async () => {
    ...
    const created = await request(app).post('/api/shipping/program').send({
      Z11_COD: code,
    ...
    const updated = await request(app).put(`/api/shipping/program/${code}`).send({
      Z11_DESC: 'Envio Atualizado'
    });
    expect(updated.status).toBe(200);
    expect(updated.body.Z11_DESC).toBe('Envio Atualizado');

    const deleted = await request(app).delete(`/api/shipping/program/${code}`);
    expect(deleted.status).toBe(204);
  });
  ...
  it('should execute marketplaces accounts CRUD flow', async () => {
    ...
    const created = await request(app).post('/api/marketplaces/accounts').send({
      Z00_COD: code,
    ...
    const updated = await request(app).put(`/api/marketplaces/accounts/${code}`).send({
      Z00_DESC: 'Conta Atualizada'
    });
    expect(updated.status).toBe(200);
    expect(updated.body.Z00_DESC).toBe('Conta Atualizada');

    const deleted = await request(app).delete(`/api/marketplaces/accounts/${code}`);
    expect(deleted.status).toBe(204);
  });
  ...
  it('should execute product x accounts flow', async () => {
    ...
    const created = await request(app).post('/api/productxaccounts').send({
    ...
    const fetched = await request(app).get(`/api/productxaccounts/${productCode}`);
    expect(fetched.status).toBe(200);
    expect(Array.isArray(fetched.body.items)).toBe(true);
    expect(fetched.body.items.length).toBe(2);

    const deleted = await request(app).delete(`/api/productxaccounts/${productCode}`);
    expect(deleted.status).toBe(204);
  });

  it('should return integrated orders details', async () => {
    const res = await request(app).get('/api/integratedorders/PED-1001/I1001');
    expect(res.status).toBe(200);
  ...
  it('should expose the same routes under /app-root/api', async () => {
    const res = await request(app).get('/app-root/api/dictionary/browse/columns/Z10');
    expect(res.status).toBe(200);
    expect(res.body.description).toBe('Plataformas');
  });
});
